...
public class testFCWithAutomaticRIC extends ROCAcceptanceTest {
	...
	@Test(priority=1, description="File Collection for Automatic Record Integrity Check")
	public void testAutomaticRIC()throws Exception
	{
		try {
			...
			ComboBoxHelper.select("FileCollection_CompressionType", "None");
			fileCollection.setRecordIntegrityCheck(true, true, null, null);
			...
			FileHelper.copyFile(applicationOS, automationPath + dataLocation, dataDir + "/Input/" + folderName, "Data_Load_Sample1.dat", "RIC_Sample1.dat", true);
			FileHelper.copyFile(applicationOS, automationPath + dataLocation, dataDir + "/Input/" + folderName, "Data_Load_Sample2.dat", "RIC_Sample2.dat", true);
