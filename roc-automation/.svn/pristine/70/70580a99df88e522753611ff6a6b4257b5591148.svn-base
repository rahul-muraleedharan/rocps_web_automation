...
public class SQLMeasureRequestHelper extends MeasureRequestHelper {
	...
	public void createMeasureRequest(String path, String workBookName, String workSheetName, String testCaseName, int occurence) throws Exception {
		try {
			...
			for(int i = 0; i < excelData.get("Measure Name").size(); i++)
			{
				...
				createMeasureRequest(partition, measureName, generateItems, description, inputMeasure, inputMeasureRequest);
			}
		} catch (Exception e) {
	...
	public void createMeasureRequest(String partition, String measureName, boolean generateItems, String description,
			String[] inputMeasure, String[] inputMeasureRequest) throws Exception {
		try {
			String detailScreenTitle = navigateToMeasureRequestDetail(partition, "SQL Measure Request", measureName);
