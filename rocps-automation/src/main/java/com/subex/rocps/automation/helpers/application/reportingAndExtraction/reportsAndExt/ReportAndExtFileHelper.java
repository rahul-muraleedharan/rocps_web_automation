...
public class ReportAndExtFileHelper extends PSAcceptanceTest
{
	...
	public void readFile( String path, String fileExtension, String reportExtDefnNm, String currentDateFormat ) throws Exception
	{
		try
		{
			String repAndExtFilePath = configProp.getDataDirPath() + configProp.getProperty( path );
			if ( ( applicationOS.equalsIgnoreCase( "Linux" ) || applicationOS.equalsIgnoreCase( "Unix" ) ) )
				readFileInLinux( repAndExtFilePath, fileExtension, reportExtDefnNm, currentDateFormat );
			else
			{
				assertTrue( FileHelper.checkDirectoryExists( repAndExtFilePath ), "The given Directory not found :" + repAndExtFilePath );

				String fileExtensionArr[] = psStringUtils.stringSplitFirstLevel( fileExtension );
				String reportExtDefnNmArr[] = psStringUtils.stringSplitFirstLevel( reportExtDefnNm );
				String currDate = DateHelper.getCurrentDateTime( currentDateFormat );//yyyyMMdd or ddMMyyyy
				int j = 0;
				while ( j < reportExtDefnNmArr.length )
				{
					for ( int i = 0; i < fileExtensionArr.length; i++ )
					{
						String fileName = reportExtDefnNmArr[j].toLowerCase() + "_" + currDate;
						String extension = fileExtensionArr[i];
						isFilePresentInDirectory( repAndExtFilePath, fileName, extension );
					}
					j++;
				}
	...
	private void isFilePresentInDirectory( String path, final String fileName, final String extension ) throws Exception
	{
		...
		FilenameFilter filter = new FilenameFilter()
		{
			...
			public boolean accept( File f, String name )
			{
				...
				boolean fileStatus = ( name.toLowerCase().endsWith( extension ) && name.toLowerCase().contains( fileName ) );
				return fileStatus;
			}
		};
	...
	private void isFilePresentInLinux( String path, String fileName, String extension, ChannelSftp channel ) throws Exception
	{
		...
		{
			if ( name.getFilename().toLowerCase().endsWith( extension ) && name.getFilename().toLowerCase().contains( fileName ) )
			{
				flag = true;
				Log4jHelper.logInfo( "File name found  :- \n " + name.getFilename() );
			}
	...
	private void readFileInLinux( String path, String fileExtension, String reportExtDefnNm, String currentDateFormat ) throws Exception
	{
		...
		try
		{
			...
			String fileExtensionArr[] = psStringUtils.stringSplitFirstLevel( fileExtension );
			String reportExtDefnNmArr[] = psStringUtils.stringSplitFirstLevel( reportExtDefnNm );
			String currDate = DateHelper.getCurrentDateTime( currentDateFormat );
			int j = 0;
			while ( j < reportExtDefnNmArr.length )
			{
				for ( int i = 0; i < fileExtensionArr.length; i++ )
