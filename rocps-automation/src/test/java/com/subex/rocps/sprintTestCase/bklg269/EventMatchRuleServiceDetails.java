...
public class EventMatchRuleServiceDetails extends PSAcceptanceTest {
	...
	protected String billProfileRadio;
	protected String billProfileComponentRadio;
	protected String billProfileComponent;
	...
	protected String[] billProfileRadioArr;
	protected String[] billProfileCompRadioArr;
	protected String[] billProfileArr;
	protected String[] billprofileCompArr; 
	protected String[] propertyValueArr;
	...
	protected void intialiseVariables(Map<String, String> map) throws Exception {
		...
		billProfileRadio=ExcelHolder.getKey(map, "BillProfileRadio");
		billProfileComponentRadio=ExcelHolder.getKey(map,"BillProfileComponentRadio");

		billProfile = ExcelHolder.getKey(map, "BillProfile");
		billProfileComponent=ExcelHolder.getKey(map,"BillProfileComponent");
		propertyValue=ExcelHolder.getKey(map,"PropertyValue");
	...
	protected void intialiseArrays() throws Exception {
		...
		billprofileCompArr=dataUtilObj.stringSplitFirstLevel(billProfileComponent);
		propertyValueArr=dataUtilObj.stringSplitFirstLevel(propertyValue);
	...
	protected void tariffConfigurationPerService(int index) throws Exception {
	 ...
	 */
	private void selectTariffs(String tffsPerServiceArr, String tffsDttmTimeLineArr) throws Exception {
	 ...
	 */
	protected void basicDetails(int index) throws Exception {
	...
	/*
	 * Method : configuring Matching Information
	 */
	protected void matchingInformation(int index) throws Exception {
		if(ValidationHelper.isFalse(billProfileComponentRadio))

		{
			String bipColHeaderName = "Bill Profile Name";			 
			EntityComboHelper.selectUsingGridFilterTextBox("//div[@id='trigger-BillProfile']", "Bill Profile Search",
					"pbipName", billProfileArr[index], bipColHeaderName);
		}

		if(ValidationHelper.isTrue(billProfileComponentRadio)){
			RadioHelper.click("tadigBasedBillProfileComponentRadioID");
			RadioHelper.click("tadigBasedBillProfileComponentRadioID");
			ComboBoxHelper.select("TADIGBasedBillProfileComponentID",billprofileCompArr[index]);
			GridHelper.clickRow("matchInformationGridID", 1, 2); // Combo box click.
            GridHelper.clickRow("matchInformationGridID", 1, 2);
            ComboBoxHelper.select(or.getProperty("propertyValueselectionID"),propertyValueArr[index]);
			
			//GridHelper.updateGridComboBox("matchingTypePanel","tclEditor_gwt_uid_",1,"Value","Primary TADIG (IN)" );
		}
	 ...
	 */
	protected void ratingDetails(int index) throws Exception {
	 ...
	 */
	protected void selectService(String rightClickXpath, String serviceId, String serviceDirection) throws Exception {
	 ...
	 */
	protected void revenueRating(int index) throws Exception {
