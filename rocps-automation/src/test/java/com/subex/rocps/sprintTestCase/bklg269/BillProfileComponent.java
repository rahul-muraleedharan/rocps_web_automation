...
public class BillProfileComponent extends PSAcceptanceTest {
		 ...
		 * in a test case
		 */
		public BillProfileComponent(String path, String workBook, String sheetName, String testCaseName) throws Exception {
			this.path = path;
		 ...
		 */
		public BillProfileComponent(String path, String workBook, String sheetName, String testCaseName, int occurence)
				throws Exception {
			this.path = path;
			this.workBook = workBook;
			this.sheetName = sheetName;
		...
		public void configureEventMatchRule() throws Exception {
			...
			try {
				...
				for (int paramVal = 0; paramVal < colSize; paramVal++) {
					...
					boolean isEventMatchRuleExists = genHelperObj.isGridTextValuePresent("PS_EvtMatchRuleName_TxtId",
							matchRuleName, emrSearchColHeader);

					if (!isEventMatchRuleExists) {
						genHelperObj.clickNewAction(partition);
						matchRuleDetailConfig();
						String matchRuleColumnHeaderName = "Name";
						assertTrue(GridHelper.isValuePresent("SearchGrid", matchRuleName, matchRuleColumnHeaderName),
								"Event match rule : " + matchRuleName + "is not saved" + testCaseName);
		 ...
		 */
		private void addingServices(MatchRuleService service) throws Exception {

			if (service == null) {
				EventMatchRuleService emrSerObj = new EventMatchRuleService();
				service = emrSerObj;
			}
			service.serviceConfiguration(mapObj);
		}
		...
		private void details() throws Exception {
			...
			String fromDateLocatorXpath = GenericHelper.getORProperty("TextBox_DatePicker").replace("idOrName",
					or.getProperty("PSDetail_emrFromDateId"));
			String toDateLocatorXpath = GenericHelper.getORProperty("TextBox_DatePicker").replace("idOrName",
					or.getProperty("PSDetail_emtToDateId"));
			...
			EntityComboHelper.selectUsingGridFilterTextBox("PSDetail_emrgEntityId", "Event Match Rule Group Search",
					"PSDetail_emrName_txtId", eventMatchRuleGroup, mrgColName);
